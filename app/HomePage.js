...
// Variante responsive pentru coverele din carduri, generate la build de
// scripts/generate-card-variants.mjs. Fara srcset, atributul `sizes` era inert:
// telefonul descarca originalul (~130KB) pentru un card afisat la ~200px.
// Generatorul produce INTOTDEAUNA ambele variante, deci nu poate lipsi un fisier.
const cardSrcSet = (cover) => {
  const base = String(cover).replace(/\.avif$/i, '')
  return `${base}-400.avif 400w, ${base}-800.avif 800w`
}
        ...
        <section style={{ background: '#081c12' }} className="pt-10 pb-10 md:pt-14 md:pb-14 px-6 text-center">
          ...
          <div className="flex items-center justify-center gap-3 mb-6">
            <a href="tel:+40758090904"
              ...
              style={{ background: '#4a6d94', color: '#fff' }}
            ...
            <a href="https://wa.me/40758090904?text=Bună ziua! Sunt interesat de apartamentele Neofort IMO."
              className="flex items-center gap-2 px-5 py-2.5 rounded-full font-semibold text-sm transition-all active:scale-95"
              style={{ background: '#1a7d33', color: '#fff' }}
