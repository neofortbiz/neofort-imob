...
export default function CalculatorClient({ faqItems = [] }) {
  ...
  useEffect(() => {
    fetch('https://api.frankfurter.app/latest?from=EUR&to=RON')
      .then(r => r.json())
      .then(d => { if (d?.rates?.RON) setCurs(d.rates.RON); else setCursError(true) })
    ...
    <div style={{ fontFamily: 'Barlow, sans-serif' }}>
      ...
      <section style={{ background: '#081c12', padding: '48px 24px 40px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, margin: 0, maxWidth: 540 }}>
      ...
      {/* CALCULATOR */}
      <section style={{ background: '#f8faf9', padding: '0 24px 48px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', overflow: 'hidden', marginTop: -24, boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ background: cursError ? '#fff7ed' : '#f0faf2', borderBottom: '1px solid #e5e7eb', padding: '8px 24px', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, color: cursLoading ? '#9ca3af' : cursError ? '#92400e' : '#2d7a3a' }}>
                {cursLoading ? 'Se actualizează cursul…' : cursError
                  ...
                  : `Curs live: 1 EUR = ${fmt(curs, 4)} RON (actualizat zilnic)`}
              </span>
            </div>
            ...
            <div style={{ padding: '28px 28px 32px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <a href="https://wa.me/40758090904?text=Bun%C4%83%20ziua!%20Vreau%20o%20simulare%20de%20credit%20ipotecar%20pentru%20un%20apartament%20Neofort."
                    target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#f0faf2', color: '#2d7a3a', border: '1px solid #c8e6c9', borderRadius: 12, padding: '11px', fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>
