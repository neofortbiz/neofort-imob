  ...
  if (match) {
    ...
    if (ansamblu?.brokerTel) {
      ...
      return {
        ...
        emailLink: `mailto:${EMAIL}?subject=${encodeURIComponent('Solicitare informații — ' + ansamblu.nume + ' (Neofort ' + ansamblu.numar + ')')}&body=${encodeURIComponent('Bună ziua,\n\nSunt interesat de apartamentele din ' + ansamblu.nume + ' (Neofort ' + ansamblu.numar + ', ' + ansamblu.zona + ', ' + ansamblu.sector + ').\n\nAș dori să primesc detalii despre disponibilitate și prețuri.\n\nVă mulțumesc.')}`,
      }
    }
  }
  const waMesaj = encodeURIComponent('Bună ziua! Sunt interesat de apartamentele Neofort IMO. Vă rog să mă contactați cu detalii despre oferta disponibilă.')
...
export default function Header({ activePath = '/' }) {
  ...
  const ContactIcons = () => (
    <div className="flex items-center gap-2 flex-shrink-0">
      ...
      <a href={contact.emailLink || `mailto:${EMAIL}`}
  ...
  return (
    <>
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[9999] focus:px-4 focus:py-2 focus:rounded-lg focus:text-sm focus:font-medium focus:text-white" style={{ background: '#2d7a3a' }}>Sari la conținut</a>
      ...
      <div className="md:hidden">
        <div className="px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href={`tel:${contact.tel}`}
              aria-label={`Sună la ${contact.telDisplay || contact.tel}`}
